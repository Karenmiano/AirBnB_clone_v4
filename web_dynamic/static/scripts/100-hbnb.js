$(document).ready(function() {
    ...
    $(".locations input:checkbox").on('change', function() {
        ...
        if (this.checked) {
            // Check if the amenity ID is not already in the array
            if (!selectedLocations.some(location => location.id === locationId)) {
                selectedLocations.push({id: locationId, name: locationName});
            }
        } else {
            // Filter out the amenity by ID
            selectedLocations = selectedLocations.filter(location => location.id !== locationId);
        }

        // Update the HTML content with the names of the selected amenities
        $(".locations h4").html(selectedLocations.map(location => location.name).join(', '));
    });
